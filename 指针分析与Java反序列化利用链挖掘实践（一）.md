...
一般情况下，对于一个网络服务，如果用户可控的数据最终被视为或封装为序列化数据，并被后端服务反序列化，那么就有可能存在反序列化漏洞的风险。对于Java而言，许多应用或组件历史上都出现过反序列化漏洞，如shiro、fastjson、xstream等。与此同时，反序列化漏洞也不仅仅存在于简单的注入场景中，像jdbc注入、jndi注入、jmx注入等等，这些攻击手法也能间接触发反序列化操作，从而达到rce等效果。

一个Java反序列化漏洞被成功利用，需要两大步骤：
1. 第一步，发现并成功触发反序列化操作
	这一步目前业内各大SAST工具已经实现得相当成熟，主要是利用污点分析，判断污点数据是否最终流入反序列化操作的关键参数
2. 第二步，发现并构造蕴含足够危害的序列化数据
	而这，将是本文重点探讨的问题
...
![](img/Pasted%20image%2020241007223935.png)
...
### 整体工作流程
整体的工作流大致如下：
![](img/Pasted%20image%2020250105110013.png)

1. IR生成。IR生成模块工作，将classpath中所有的类处理为中间表示，最终得到一个IR数据库
